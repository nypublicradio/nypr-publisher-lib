...
module('Integration | Component | show tease', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    await render(
      hbs`{{show-tease show=testShow featuredSpot=featured trackingCategory="Test Category" simpleLayout=simpleLayout}}`
    );
    ...
    assert.equal(this.$('.flag-image img').attr("src"), "http://www.wnyc.org/i/90/90/l/85/1/2DQ_1400X1400_NoWNYCSTUDIOS_2.png", "Image is populated");
    ...
    assert.equal(this.$('.flag-body .text h4 a').length, 0, 'Producing orgs should not have links');
    assert.equal(this.$('.flag-body .text h2').text().trim(), 'All Things Considered', 'Title should render correctly');
    assert.equal(this.$('.flag-body .text .text--default').text().trim(), "A wrap-up of the day's news, with features and interviews.", 'Description should render correctly');
    assert.equal(this.$('.flag-image img').attr("src"), "http://www.wnyc.org/i/150/150/l/85/1/2DQ_1400X1400_NoWNYCSTUDIOS_2.png", "Image is populated");
    assert.equal(this.$('.flag-body .text h2 a').attr("data-tracking-category"), "Test Category", "Tracking Category should render on title link");
    assert.equal(this.$('a.flag-image').attr("data-tracking-category"), "Test Category", "Tracking Category should render on image link" );
